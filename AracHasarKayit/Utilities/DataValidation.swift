...
class DataValidationManager {
    ...
    func validate<T: DataValidator>(_ object: T) -> String? {
        do {
            try object.validate()
            return nil
    ...
    func validateBatch<T: DataValidator>(_ objects: [T]) -> [String] {
        var errors: [String] = []
        
        for object in objects {
            if let error = validate(object) {
                errors.append(error)
            }
        }
    ...
    func checkDuplicatePlate(_ plate: String, in vehicles: [Arac], excluding: UUID? = nil) -> Bool {
        let cleanPlate = plate.replacingOccurrences(of: " ", with: "").uppercased()
        
        return vehicles.contains { arac in
            if let excludeId = excluding, arac.id == excludeId {
                return false
            }
            
            let existingPlate = arac.plaka.replacingOccurrences(of: " ", with: "").uppercased()
            return existingPlate == cleanPlate
        }
    }
    
    /// Check for duplicate RES code
    func checkDuplicateRESCode(_ resCode: String, in vehicle: Arac, excluding: UUID? = nil) -> Bool {
        return vehicle.hasarKayitlari.contains { hasar in
            if let excludeId = excluding, hasar.id == excludeId {
                return false
            }
            return hasar.resKodu.uppercased() == resCode.uppercased()
        }
    }
}

// MARK: - SwiftUI Validation Helpers
...
extension View {
    func validationAlert(_ errorMessage: Binding<String?>) -> some View {
        modifier(ValidationAlert(errorMessage: errorMessage))
    }
}
