...
struct IadeDetayView: View {
    ...
    private func sendReturnEmail() {
        ...
        IadePDFGenerator.shared.generateIadePDF(iade: liveIade, arac: arac, franchiseDisplayName: viewModel.franchiseName) { localURL in
            ...
            uploadReturnPDFWithRetry(data: data, path: pdfPath) { uploadedPDFURL in
                guard let uploadedPDFURL else { finishEmailFlow(success: false, message: "PDF upload failed.".localized); return }
                ...
                FirebaseService.shared.queueReturnEmail(
                    ...
                    if let error { print("❌ Queue error: \(error.localizedDescription)"); finishEmailFlow(success: false, message: "Email queue failed.".localized); return }
                    guard let documentPath = queuedPaths.first else { finishEmailFlow(success: false, message: "Email queue path missing.".localized); return }
    ...
    private func observeQueuedEmailStatus(documentPath: String, timeout: TimeInterval = 45, completion: @escaping (String) -> Void) {
        ...
        var registration: ListenerRegistration?; var didComplete = false
        func finish(_ status: String) {
            guard !didComplete else { return }
            didComplete = true; registration?.remove(); registration = nil; completion(status)
        }
        registration = ref.addSnapshotListener { snapshot, error in
            if let error { print("❌ Listener error: \(error.localizedDescription)"); finish("listener_error"); return }
