        ...
        VStack(spacing: 0) {
            ScrollView {
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                    ForEach(OfficeOperationType.allCases) { opType in
...
struct AllOfficeOperationsReportView: View {
    ...
    var filteredOperations: [OfficeOperation] {
        ...
        return viewModel.officeOperations.filter { $0.date >= dateRange.start && $0.date <= dateRange.end }
    }
    ...
    func createPDFData() -> Data {
        let pdfMetadata = [
            kCGPDFContextTitle: "Overall Office Operations Report",
            kCGPDFContextAuthor: "Green Motion AG"
        ]
        let format = UIGraphicsPDFRendererFormat()
        format.documentInfo = pdfMetadata as [String: Any]
        ...
        return renderer.pdfData { context in
            ...
            // Title
            let title = "Overall Office Operations Report"
            title.draw(at: CGPoint(x: 50, y: yPosition), withAttributes: [.font: titleFont])
            yPosition += 40
            ...
            // Breakdown by Type
            "Breakdown by Type:".draw(at: CGPoint(x: 50, y: yPosition), withAttributes: [.font: headingFont])
            yPosition += 25
            
            for item in operationsByType.prefix(10) {
                let line = "\(item.type.rawValue): \(item.count) entries - \(String(format: "%.2f CHF", item.amount))"
                line.draw(at: CGPoint(x: 70, y: yPosition), withAttributes: [.font: bodyFont])
                yPosition += 20
    ...
    func createCSVData() -> Data {
        ...
        for operation in filteredOperations.sorted(by: { $0.date > $1.date }) {
            ...
            let amountStr = String(format: "%.2f", operation.amount)
            let notes = operation.notes.replacingOccurrences(of: ",", with: ";")
            csv += "\(dateStr),\(operation.type.rawValue),\(amountStr),\(notes)\n"
        }
